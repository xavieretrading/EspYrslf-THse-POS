...
class Layout {
  ...
  twoColumnRow(left: string, right: string, minLeftWidth = 20): string {
    const defaultColW = Math.floor(this.width / 2);
    let leftWidth = defaultColW;
    if (right.length > (this.width - defaultColW)) {
      leftWidth = Math.max(left.length + 2, minLeftWidth);
    }
    const l = rpad(left, leftWidth);
    const maxRight = this.width - l.length;
    const r = right.length > maxRight ? right.substring(0, maxRight) : right;
    return this.clip(l + r);
  }
...
const PRESETS: Record<ReceiptTemplateId, Preset> = {
  classic: { spacer: true, div: '-', unitPriceLine: false, heroTotal: false, minimal: false },
  compact: { spacer: false, div: '-', unitPriceLine: false, heroTotal: false, minimal: false },
  elegant: { spacer: true, div: '=', unitPriceLine: false, heroTotal: true, title: 'RECEIPT', minimal: false },
...
function buildLines(data: LayoutData, options: ReceiptRenderOptions): StyledLine[] {
  ...
  const lv = (label: string, value: string, bold = false) => {
    for (const ln of L.labelValue(label, value)) push(ln.replace(/₱/g, 'PHP '), 'l', bold ? { bold: true } : {});
  };
  if (f.orderNumber && f.dateTime && f.server) {
    push(L.twoColumnRow(`Order: #${data.orderNumber}`, `Date: ${data.date}`), 'l');
    push(L.twoColumnRow(data.time ? `Time: ${data.time}` : '', `Cashier: ${data.server}`), 'l');
  } else {
    if (f.orderNumber) lv('Order', `#${data.orderNumber}`, true);
    if (f.dateTime) { lv('Date', data.date); if (data.time) lv('Time', data.time); }
    if (f.server && data.server) lv('Cashier', data.server);
  }
  ...
  for (const it of data.items) {
    const cleanItemName = it.name.replace(/₱/g, 'PHP ');
    for (const ln of L.itemRow(cleanItemName, String(it.quantity), money(it.total))) push(ln, 'l');
    if (p.unitPriceLine && f.unitPrice) for (const ln of L.indented(`${it.quantity} x ${money(it.unitPrice)}`, 2)) push(ln, 'l');
    if (f.itemModifiers && it.modifiers?.length) for (const m of it.modifiers) for (const ln of L.indented(m.replace(/₱/g, 'PHP '), 2, '+')) push(ln, 'l');
    if (f.itemNotes && it.notes) {
      const cleanNote = it.notes
        .replace(/Service:\s*[^|]+(?:\s*\|\s*Weight[\s/]*Qty:[^|,]+)?(?:\s*,?\s*Rate:[^|•]+)?\.?/gi, '')
        .replace(/Weight[\s/]*Qty:[^|,]+(?:\s*,?\s*Rate:[^|•]+)?\.?/gi, '')
        .replace(/₱/g, 'PHP ')
        .trim();
      if (cleanNote && !cleanNote.toLowerCase().startsWith('service:') && !(cleanNote.toLowerCase().includes('weight') && cleanNote.toLowerCase().includes('rate'))) {
        for (const ln of L.indented(cleanNote, 2, '*')) push(ln, 'l');
      }
    }
  }
  divider();
